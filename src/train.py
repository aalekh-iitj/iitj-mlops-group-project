...
from ._logging import quiet_cuda_warnings
from .data import prepare_dataset
from .utils import get_env, load_id2label, load_label2id

# Silence harmless cuFFT/cuDNN/cuBLAS "factory already registered" noise on Kaggle.
...
def main():
    ...
    parser.add_argument(
        "--train_csv", required=True, help="Path to processed train.csv"
    )
    ...
    parser.add_argument(
        "--push_to_hub", action="store_true", help="Push best model to HF Hub"
    )

    parser.add_argument(
        "--hub_repo_id", default=None, help="e.g. your-username/mlops-a3-model"
    )
    ...
    if args.push_to_hub and args.hub_repo_id:
        ...
        wandb.run.summary["huggingface_model"] = (
            f"https://huggingface.co/{args.hub_repo_id}"
        )
